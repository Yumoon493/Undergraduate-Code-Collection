...
class BertForTokenClassification(nn.Layer):
    def __init__(self, bert, num_classes=2):  # Remove dropout parameter
        super(BertForTokenClassification, self).__init__()
        self.num_classes = num_classes
        self.bert = bert
        # Drop dropout layer, Connect BERT directly to the classification layer
    ...
    def forward(self,
        ...
        # Dropout processing is not required
...
model = AutoModelForTokenClassification.from_pretrained(
    model_name,
    num_classes=5,
    hidden_dropout_prob=0.0,  # Disable the Dropout inside BERT
    attention_probs_dropout_prob=0.0  # Disable the Dropout of the attention mechanism
)
...
num_training_steps = len(train_loader) * num_epochs # Total number of training steps required
